...
contract DailyPool is ReentrancyGuard, Ownable {
  ...
  function claimPrize(bytes calldata signature, uint256 _prize) external nonReentrant {
    require(_verifyPlatformOwner(signature, _prize), "Wrong prize or signature");
    require(roundUserInfos[currentRoundId - 1][msg.sender].isAnswered, "Not answered for this round");
